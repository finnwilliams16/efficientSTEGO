...
from semantic_head.depthwise_seperable_conv import DepthwiseSeparableConv
from semantic_head import SemanticHead

class DPC_stego(nn.Module):
    def __init__(self):
        super().__init__()
        options = {
            'in_channels'   : 256,
        ...
        self.connector = torch.nn.Conv2d(384, 256, (1, 1))
        ...
        self.conv_branch_3 = DepthwiseSeparableConv(dilation=(18, 15),
                                                    padding=(18, 15),
                                                    **options)
        self.iabn_branch_3 = InPlaceABN(256)        # Branch 4
    ...
    def forward(self, inputs):
        ...
        outputs = self.efficientPS_to_STEGO(outputs)
        return outputs
...
class Cat(nn.Module):
    ...
    def forward(self, x):
        to_concat = []
        to_concat.append(x)
        [to_concat.append(i) for i in self.tensors]
        x = self.concat(tensors=to_concat, dim=self.dim)
        return x
...
class DinoFeaturizer(nn.Module):
    ...
    def __init__(self, dim, cfg):
        ...
        if self.proj_type == "nonlinear":
            if cfg.head == "original_stego":
                self.cluster2 = self.make_nonlinear_clusterer(self.n_feats)
            elif cfg.head == "lsfe":
                self.cluster2 = nn.Sequential(
                                    torch.nn.Conv2d(384, 256, (1, 1)),
                                    DepthwiseSeparableConv(256, 128, 3, padding=1),
                                    DepthwiseSeparableConv(128, 128, 3, padding=1),
                                    InPlaceABN(128),
                                    torch.nn.Conv2d(128, 70, (1, 1)))
            elif cfg.head == "mc":
                self.cluster2 = nn.Sequential(
                                    torch.nn.Conv2d(384, 128, (1, 1)),
                                    DepthwiseSeparableConv(128, 128, 3, padding=1),
                                    DepthwiseSeparableConv(128, 128, 3, padding=1),
                                    InPlaceABN(128),
                                    Interpolate(scale_factor=(2, 2), mode='bilinear', align_corners=False), # Interpolate upsamples
    ...
    def forward(self, img, n=1, return_class_feat=False):
        ...
        if self.proj_type is not None:
            ...
            if self.proj_type == "nonlinear":
                if self.cfg.head == "dpc":
                    dpc_unit = DPC_stego().to(torch.device("cuda:0"))
                    code += dpc_unit.forward(self.dropout(image_feat.to(torch.device("cuda:0"))))

                elif self.cfg.head == "full":
                    full_sem_head = SemanticHead().to(torch.device("cuda:0"))
                    code += full_sem_head.forward(self.dropout(image_feat.to(torch.device("cuda:0"))))
                else:
