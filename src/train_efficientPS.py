...
class CombinedModel(pl.LightningModule):
  def __init__(self, body, connector, head):
    super().__init__()
    self.body = body
...
class LitUnsupervisedSegmenter(pl.LightningModule):
    def __init__(self, n_classes, cfg):
        ...
        self.efficientPS_metrics = UnsupervisedMetrics(
            "efficientPS", n_classes, cfg.extra_clusters, True)
        ...
        self.head = SemanticHead(19)
        self.connector = Connector()
        #body = nn.Sequential(*list(self.net(x)[1].children())[:-12]) # Delete seg head from STEGO
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            z = self.connector(code)
            yhat = self.head(z)
            efficientPS_preds = yhat.argmax(1)
            self.efficientPS_metrics.update(efficientPS_preds, label)

            return {
                'img': img[:self.cfg.n_images].detach().cpu(),
                #'linear_preds': linear_preds[:self.cfg.n_images].detach().cpu(),
                #"cluster_preds": cluster_preds[:self.cfg.n_images].detach().cpu(),
                "efficientPS_preds": efficientPS_preds[:self.cfg.n_images].detatch().cpu(),
    ...
    def validation_epoch_end(self, outputs) -> None:
        ...
        with torch.no_grad():
            tb_metrics = {
                #**self.linear_metrics.compute(),
                #**self.cluster_metrics.compute(),
                **self.efficientPS_metrics.compute()
            }
            ...
            self.efficientPS_metrics.reset()
    ...
    def configure_optimizers(self):
        ...
        efficient_ps_optim = torch.optim.Adam(list(self.head.parameters()), lr=5e-3)

        return efficient_ps_optim
